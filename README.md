...
Lume started as a fork of `fstguardrails` by [Steve Harris](https://github.com/jsclosures) (`jsclosures`), an OG search expert with a legendary systems pedigree. Before building massive web-scale search solutions, Steve served as a **U.S. Marine Corps Air Traffic Controller** (Sgt MATCS-38 Det C)—an environment where split-second, high-reliability deterministic routing is literally a matter of life and death. 

After his military service, Steve earned his CS degree from **Washington State University** (focusing on 2D and 3D graphics rendering systems) and interned at **Apple** in the mid-90s. He later worked as a **Sphere Lucene Developer at AOL**, building core natural language understanding and search retrieval systems, before spending over two decades as a Senior Solutions Architect at **Portaltown**. 

This lifelong training in high-stakes deterministic paths and rendering pipelines culminated in his zero-dependency JavaScript finite-state tagger (`lib.js`). In information retrieval, the **Finite State Transducer (FST)** is a legendary primitive: it compiles massive dictionaries of phrases into highly compressed, deterministically navigable byte networks. Steve's FST tagger demonstrated how to perform longest-match phrase tagging, ASCII diacritic folding, and hyphen stripping with zero runtime dependencies and extreme memory efficiency. While FST matching is ideal for enforcing safety guardrails, it only tells you *what* terms are in your documents. To build a complete search engine, this routing layer needed a ranking and indexing framework.

### Systems and Advertising Scale (Kord's Backstory)
[Kord Campbell](https://github.com/kordless) (`kordless`) came to search from a deep systems, log-analysis, and web-crawling background. In 2000, he founded **Grub**, a pioneering peer-to-peer distributed web crawler (later acquired by LookSmart and Wikia/Fandom), and subsequently co-founded the log-search platform **Loggly**. 

Having deployed and optimized massive Lucene, Solr, and Splunk clusters over the decades, Kord's engineering approach is decidedly low-key and intuitive: *"I don't see it, I feel it. I understand the primitive and pull from those that I see are 'right' for the job. That is just good pattern matching."* Rather than relying on rigid, pre-packaged software stacks, he works by feel—synthesizing fundamental mathematical and architectural primitives at a bare-metal level.

This philosophy is what brought **Roaring Bitmaps** into Lume. In search advertising (SEO and ad networks), systems must calculate intersections of massive document and user segment lists in microseconds to serve the right ads. This extreme environment demands roaring bitmaps—a highly compressed representation of integer arrays that performs bitwise set math at hardware speed. By fusing **Steve's FST phrase matching** with **Kord's intuitive systems-level set intersections** and traditional **BM25 lexical ranking**, Lume was born—a modern search engine constructed by layering clean, high-performance primitives directly on top of the FST word tree.
